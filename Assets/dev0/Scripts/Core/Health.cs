...
public class Health : MonoBehaviour
{
    ...
    //public bool rageWhenAlmostDie = false;
    ...
    public int rageCount = 1;
    public int currRageCount = 0;

    [Header("Events")]
    public UnityEvent<int> onDeath; // Event untuk ketika mati (dengan parameter score)
    public UnityEvent<int> onRage;
    ...
    private void Die()
    {
        ...
        else if (haveDeathscreen)
        {
            ...
            showDeathsreen();
        } else if(canRage && currRageCount < rageCount)
        {
            currRageCount += 1;
            onRage?.Invoke(currRageCount);
        }
        else if(canRage && currRageCount == rageCount)
        {
            onDeath?.Invoke(CalculateScoreValue() * rageCount);
        }
        else
        {
            StartCoroutine(MoveToBottom());
